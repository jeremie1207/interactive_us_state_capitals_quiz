...
def sample_question() -> Question:
    return Question(
        ...
        "Sacramento",
        {"A": "Phoenix", "B": "Sacramento", "C": "Denver", "D": "Austin"},
    )
...
def test_set_user_answer_question_error(sample_question: Question) -> None:
    with pytest.raises(
        ValueError,
        match=r"The answer should be A, B, C, or D",
    ):
...
def test_calculate_score_quiz(
    sample_quiz: Quiz, sample_question: Question
) -> None:
    sample_question.set_user_answer("B")
...
def test_get_score_in_percent_quiz(
    sample_quiz: Quiz, sample_question: Question
) -> None:
    sample_question.set_user_answer("B")
    sample_quiz.questions = [sample_question]
